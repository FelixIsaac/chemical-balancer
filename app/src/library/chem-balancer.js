...
function buildMatrix(eqn) {
  const elems = eqn.getElements();
  const rows = elems.length + 1;
...
function checkAnswer(eqn, coefs) {
  ...
  for (let i = 0; i < coefs.length; i++) {
    const coef = coefs[i];
    if (typeof coef !== 'number' || isNaN(coef) || Math.floor(coef) !== coef) throw 'Assertion error: Not an integer';
    allzero &= coef === 0;
  }
...
// A complete chemical equation has a LHS list of terms and a RHS list of terms.
    ...
    function termsToHtml(terms) {
      ...
      for (let i = 0; i < terms.length; i++, j++) {
        ...
        if (coef !== 0) {
          ...
          if (coef !== 1) node.appendChild(createSpan(coef.toString().replace(/-/, MINUS), 'coefficient'));
...
function Term(items, charge) {
  if (items.length === 0 && charge !== -1) throw 'Invalid term'; // Electron case

  this.getItems = function () { return cloneArray(items); };

  this.getElements = function (resultSet) {
    resultSet.add('e');
    for (let i = 0; i < items.length; i++) items[i].getElements(resultSet);
  };
  ...
  this.countElement = function (name) {
    ...
    let sum = 0;
    for (let i = 0; i < items.length; i++) sum = checkedAdd(sum, items[i].countElement(name));
    return sum;
  };
...
function Group(items, count) {
  if (count < 1) throw 'Assertion error: Count must be a positive integer';
...
function parse(formulaStr) {
  try {
    ...
    if (typeof err === 'string') { // Simple error message string
      ...
      // Adjust position to eliminate whitespace
...
function parseEquation(tok) {
  ...
  while (true) {
    ...
    if (next === '=') {
      ...
      tok.consume('+');
      lhs.push(parseTerm(tok));
    } else throw { message: 'Plus expected', start: tok.position() };
  }
  ...
  while (true) {
    ...
    if (next == null) break;
      ...
      tok.consume('+');
      rhs.push(parseTerm(tok));
    } else throw { message: 'Plus or end expected', start: tok.position() };
  }
...
function parseTerm(tok) {
  const startPosition = tok.position();
  ...
  while (true) {
    ...
    if (next == null) break;
    else if (next === '(') items.push(parseGroup(tok));
    else if (/^[A-Za-z][a-z]*$/.test(next)) items.push(parseElement(tok));
    else break;
  }
  ...
  if (next != null && next === '^') {
    tok.consume('^');
    next = tok.peek();
    if (next == null) throw { message: 'Number or sign expected', start: tok.position() };
    else charge = parseOptionalNumber(tok);

    next = tok.peek();
    if (next === '+') charge = +charge; // No-op
    else if (next === '-') charge = -charge;
    else throw { message: 'Sign expected', start: tok.position() };
    tok.take(); // Consume the sign
  }

  // Check if term is valid
  let elems = new Set();
  for (let i = 0; i < items.length; i++) items[i].getElements(elems);
  ...
  if (items.length === 0) {
    ...
    if (items.length > 1) throw { message: 'Invalid term - electron needs to stand alone', start: startPosition, end: tok.position() };
    else if (charge !== 0 && charge !== -1) throw { message: 'Invalid term - invalid charge for electron', start: startPosition, end: tok.position() };
    // Tweak data
    items = [];
    charge = -1;
  } else { // Otherwise, a term must not contain an element that starts with lowercase
    for (let i = 0; i < elems.length; i++) {
      if (/^[a-z]+$/.test(elems[i])) throw { message: `Invalid element name "${elems[i]}"`, start: startPosition, end: tok.position() };
    }
  }
...
function parseGroup(tok) {
  ...
  while (true) {
    ...
    if (next == null) throw { message: 'Element, group, or closing parenthesis expected', start: tok.position() };
    else if (next === '(') items.push(parseGroup(tok));
    else if (/^[A-Za-z][a-z]*$/.test(next)) items.push(parseElement(tok));
    else if (next === ')') {
      tok.consume(')');
      if (items.length === 0) throw { message: 'Empty group', start: startPosition, end: tok.position() };
      break;
    } else throw { message: 'Element, group, or closing parenthesis expected', start: tok.position() };
  }
...
function parseOptionalNumber(tok) {
  const next = tok.peek();
  if (next != null && /^[0-9]+$/.test(next)) return checkedParseInt(tok.take());
  return 1;
}
...
function Tokenizer(str) {
  ...
  this.peek = function () {
    if (i === str.length) return null; // End of stream

    const match = /^([A-Za-z][a-z]*|[0-9]+|[+\-^=()])/.exec(str.substring(i));
    if (match == null) throw { message: 'Invalid symbol', start: i };
    return match[0];
  };
  ...
  this.consume = function (s) {
    if (this.take() !== s) throw 'Token mismatch';
  };

  function skipSpaces() {
    const match = /^[ \t]*/.exec(str.substring(i));
    i += match[0].length;
  }
...
function Matrix(rows, cols) {
  if (rows < 0 || cols < 0) throw 'Illegal argument';

  // Initialize with zeros
  let row = [];
  for (let j = 0; j < cols; j++) row.push(0);
  const cells = []; // Main data (the matrix)
  for (let i = 0; i < rows; i++) cells.push(cloneArray(row));
  row = null;
  ...
  this.get = function (r, c) {
    if (r < 0 || r >= rows || c < 0 || c >= cols) throw 'Index out of bounds';
    return cells[r][c];
  };
  ...
  // Swaps the two rows of the given indices in this matrix, degenerate case of i == j is allowed.
  function swapRows(i, j) {
    if (i < 0 || i >= rows || j < 0 || j >= rows) throw 'Index out of bounds';
    const temp = cells[i];
    cells[i] = cells[j];
    cells[j] = temp;
  }
  ...
  // The rows are not indices. This object's data is unused.
  // For example, addRow([3, 1, 4], [1, 5, 6]) = [4, 6, 10].
  function addRows(x, y) {
    const z = cloneArray(x);
    for (let i = 0; i < z.length; i++) z[i] = checkedAdd(x[i], y[i]);
    return z;
  }

  // Returns a new row that is the product of the given row with the given scalar.
  // The row is is not an index. This object's data is unused.
  // For example, multiplyRow([0, 1, 3], 4) = [0, 4, 12].
  function multiplyRow(x, c) {
    const y = cloneArray(x);
    for (let i = 0; i < y.length; i++) y[i] = checkedMultiply(x[i], c);
    return y;
  }
  ...
  function gcdRow(x) {
    let result = 0;
    for (let i = 0; i < x.length; i++) result = gcd(x[i], result);
    return result;
  }
  ...
  // Changes this matrix to reduced row echelon form (RREF), except that each leading
  // coefficient is not necessarily 1. Each row is simplified.
